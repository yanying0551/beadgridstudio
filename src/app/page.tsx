...
export default function Home() {
  ...
  return (
    <main className="mx-auto min-h-screen max-w-7xl overflow-x-hidden px-4 py-10 sm:px-6 lg:px-8">
      ...
      <section aria-label="Pattern settings" className="mt-8 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
      ...
      {pattern ? (
        <div className="mt-8 grid min-w-0 gap-6 xl:grid-cols-[minmax(0,1fr)_20rem]">
          ...
          <aside className="grid content-start gap-6"><PatternMaterials document={pattern} /><PatternExportPanel document={pattern} /></aside>
        </div>
      ) : <div className="mt-8 max-w-sm"><PatternExportPanel /></div>}
    </main>
  );
}
